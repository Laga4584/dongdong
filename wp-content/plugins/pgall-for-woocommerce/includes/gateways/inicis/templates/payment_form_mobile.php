...
<form id="form1" name="ini" method="post" action="" accept-charset="EUC-KR">
    ...
    <input type="hidden" name="P_AMT" value="<?php echo intval( $order->get_total() ); ?>"/>
    <input type="hidden" name="P_UNAME" value="<?php echo pafw_remove_emoji( pafw_get_object_property( $order, 'billing_last_name' ) . pafw_get_object_property( $order, 'billing_first_name' ) ); ?>"/>
	...
	<?php if ( wc_tax_enabled() ) : ?>
        <input type="hidden" name="P_TAX" value="<?php echo intval( $tax_amount ); ?>">
        <input type="hidden" name="P_TAXFREE" value="<?php echo intval( $tax_free_amount ); ?>">
	<?php endif; ?>
