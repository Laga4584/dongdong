...
if ( class_exists( 'WC_Payment_Gateway' ) ) {
	...
	class WC_Gateway_Inicis extends PAFW_Payment_Gateway {
		...
		public static function enqueue_frontend_script() {
			wp_enqueue_script( 'pafw-inicis', 'https://stdpay.inicis.com/stdjs/INIStdPay.js' );
		}
		...
		function make_hash( $order, $txnid, $print_log = false ) {
			$order_total        = $order->get_total();
			$billing_first_name = pafw_remove_emoji( pafw_get_object_property( $order, 'billing_first_name' ) );
			...
			$str = (string) $this->merchant_id . "|$txnid|$order_total|$product_info|" . $billing_first_name . "|" . $billing_email . "|||||||||||";
		...
		function get_order_from_notification( $notification ) {
			...
			if ( null == $order = apply_filters( 'pafw_get_order', null, $this->get_order_id_from_txnid( $notification->txnid ) ) ) {
				...
				$hash = $this->make_hash( $order, $notification->txnid );
		...
		function get_payment_form( $order_id = null, $order_key = null ) {
			try {
				...
				$hash         = $this->make_hash( $order, $txnid );
				$notification = $this->encrypt_notification( $txnid, $hash );

				$tax_amount      = $this->get_total_tax( $order );
