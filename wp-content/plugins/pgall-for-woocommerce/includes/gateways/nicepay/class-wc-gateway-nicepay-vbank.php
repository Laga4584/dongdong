	...
	if ( ! class_exists( 'WC_Gateway_Nicepay_Vbank' ) ) {
		...
		class WC_Gateway_Nicepay_Vbank extends WC_Gateway_Nicepay {
			...
			public function process_standard( $order, $nicepay ) {
				pafw_update_meta_data( $order, '_pafw_vacc_tid', $nicepay->m_ResultData['TID'] );
				pafw_update_meta_data( $order, '_pafw_vacc_num', trim( $nicepay->m_ResultData['VbankNum'] ) );  //입금계좌번호
				pafw_update_meta_data( $order, '_pafw_vacc_bank_code', trim( $nicepay->m_ResultData['VbankBankCode'] ) );    //입금은행코드
				pafw_update_meta_data( $order, '_pafw_vacc_bank_name', pafw_convert_to_utf8( $nicepay->m_ResultData['VbankBankName'] ) );    //입금은행명/코드
				pafw_update_meta_data( $order, '_pafw_vacc_holder', '' );    //예금주
				pafw_update_meta_data( $order, '_pafw_vacc_depositor', pafw_convert_to_utf8( $nicepay->m_ResultData['BuyerName'] ) );   //송금자
				pafw_update_meta_data( $order, '_pafw_vacc_date', trim( $nicepay->m_ResultData['VbankExpDate'] ) );    //입금예정일
