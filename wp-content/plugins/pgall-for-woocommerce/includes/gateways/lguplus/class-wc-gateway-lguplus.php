...
if ( class_exists( 'WC_Payment_Gateway' ) ) {
	class WC_Gateway_Lguplus extends PAFW_Payment_Gateway {
		...
		public static function enqueue_frontend_script() {
			wp_enqueue_script( 'pafw-lguplus', 'https://xpay.uplus.co.kr/xpay/js/xpay_crossplatform.js' );
		}
		...
		function get_payment_form( $order_id, $order_key ) {
			try {
				...
				$LGD_TAXFREEAMOUNT = $this->get_tax_free_amount( $order );
				$LGD_TIMESTAMP     = date( 'YmdHis' );                         //타임스탬프
				$LGD_BUYER         = pafw_remove_emoji( pafw_get_object_property( $order, 'billing_last_name' ) . pafw_get_object_property( $order, 'billing_first_name' ) );
