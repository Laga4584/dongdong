			...
			function process_vbank_noti() {
				...
				try {
					...
					if ( 'I' != $_REQUEST['LGD_CASFLAG'] ) {
						throw new Exception( sprintf( __( '[PAFW-ERR-8004] 입금 통보가 아닙니다. [%s]', 'pgall-for-woocommerce' ), $_REQUEST['LGD_CASFLAG'] ) );
					}
					...
					if ( $_REQUEST['LGD_TID'] != $this->get_transaction_id( $order ) ) {
						throw new Exception( sprintf( __( '[PAFW-ERR-8004] TID 불일치 [%s] [%s]', 'pgall-for-woocommerce' ), $_REQUEST['LGD_TID'], $this->get_transaction_id( $order ) ) );
					}
