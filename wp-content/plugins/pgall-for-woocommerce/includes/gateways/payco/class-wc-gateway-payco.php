	...
	class WC_Gateway_Payco extends PAFW_Payment_Gateway {
		...
		function cancel_request( $order, $msg, $code = "1" ) {
			$params = array (
				...
				'totalCancelTaxfreeAmt' => $this->get_tax_free_amount( $order ),
				'totalCancelTaxableAmt' => $this->get_tax_amount( $order ),
				'totalCancelVatAmt'     => $this->get_total_tax( $order ),
		...
		function process_payment( $order_id ) {
			...
			$params = array (
				...
				'totalTaxfreeAmt'             => $this->get_tax_free_amount( $order ),
				'totalVatAmt'                 => $this->get_total_tax( $order ),
				'totalTaxableAmt'             => $this->get_tax_amount( $order ),
				'currency'                    => 'KRW',
		...
		public function process_refund( $order_id, $amount = null, $reason = '' ) {
			...
			if ( $order ) {
				$params = array (
					'sellerKey'       => $this->seller_key,
					'orderNo'         => $this->get_transaction_id( $order ),
					'cancelTotalAmt'  => $amount,
					'totalCancelTaxfreeAmt' => $this->get_tax_free_amount( $order, $amount ),
					'totalCancelTaxableAmt' => $this->get_tax_amount( $order, $amount ),
					'totalCancelVatAmt'     => $this->get_total_tax( $order, $amount ),
					'orderCertifyKey' => pafw_get_meta( $order, '_pafw_order_certify_key' ),
					'requestMemo'     => $reason
				);
				...
				if ( 0 == $response['code'] ) {
					...
					$cancel_details = current( $response['result']['cancelPaymentDetails'] );

					$this->add_payment_log( $order, '[ 결제 취소 성공 ]', array (
						'거래요청번호' => $cancel_details['cancelPaymentTradeNo'],
