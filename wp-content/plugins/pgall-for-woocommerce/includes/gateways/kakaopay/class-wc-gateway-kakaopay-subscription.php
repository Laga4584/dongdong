			...
			public function subscription_additional_charge() {
				...
				if ( $order ) {
					...
					if ( ! empty( $response['aid'] ) ) {
						...
						$this->add_payment_log( $order, '[ 추가 과금 오류 ] ', json_encode( $response, JSON_UNESCAPED_UNICODE ), false );
						throw new PAFW_Exception( "[ 추가 과금 오류 ]\n" . json_encode( $response, JSON_UNESCAPED_UNICODE ), '2004', $response['code'] );
					}
				...
				try {
					...
					if ( $order && $subscriptions ) {
						...
						if ( ! empty( $response['aid'] ) ) {
							...
							throw new PAFW_Exception( json_encode( $response, JSON_UNESCAPED_UNICODE ), '2004', $response['code'] );
						}
					}
				} catch ( Exception $e ) {
					$error_code = '';
