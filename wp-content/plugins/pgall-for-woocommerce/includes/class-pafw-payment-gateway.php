...
if ( class_exists( 'WC_Payment_Gateway' ) ) {
	...
	abstract class PAFW_Payment_Gateway extends WC_Payment_Gateway {
		...
		function __get( $key ) {
			return isset( $this->settings[ $key ] ) ? $this->settings[ $key ] : ( property_exists( $this, $key ) ? $this->$key : '' );
		}
		...
		public function is_refundable( $order, $screen = 'admin' ) {
			if ( 'admin' == $screen ) {
				return ! in_array( $order->get_status(), array ( 'refunded', 'cancelled' ) );
			} else {
		...
		public function get_bill_key( $order, $is_renewal = false ) {
			...
			if ( is_a( $order, 'WC_Order' ) && ( empty( $bill_key ) || 'subscription' == $management_batch_key ) ) {
				$subscription = false;
		...
		public function subscription_additional_charge() {
			try {
				...
				if ( $order ) {
					...
					$response = $this->request_subscription_payment( $order, $amount_to_charge, $params );
					...
					$this->add_payment_log( $order, '[ 추가 과금 성공 ]', array (
						...
						'추가과금금액' => wc_price( $amount_to_charge, array ( 'currency' => $order->get_currency() ) )
					) );
			...
			try {
				...
				if ( function_exists( 'wcs_is_subscription' ) && wcs_is_subscription( $order ) ) {
					...
					if ( $this->id == $order->get_payment_method() ) {
						try {
							$bill_key = pafw_get_meta( $order, $this->get_subscription_meta_key( 'bill_key' ) );

							if ( ! empty( $bill_key ) ) {
								$this->cancel_bill_key( $bill_key );

								$this->clear_bill_key( $order, pafw_get_object_property( $order, 'customer_id' ) );
					...
					$_REQUEST['card_quota'] = pafw_get( $_REQUEST, 'pafw_' . $this->master_id . '_card_quota', '00' );
				...
				if ( $order ) {
					...
					if ( empty( pafw_get_object_property( $order, 'paid_date' ) ) ) {
						$order->update_status( 'failed', __( '결제내역을 확인하신 후, 고객에게 연락을 해주시기 바랍니다.', 'pgall-for-woocommerce' ) );
					}
				}
		...
		function woocommerce_scheduled_subscription_payment( $amount_to_charge, $order ) {
			try {
				$this->request_subscription_payment( $order, $amount_to_charge, array ( 'is_renewal' => true ) );
			} catch ( Exception $e ) {
				$message = sprintf( __( '[PAFW-ERR-%s] %s', 'pgall-for-woocommerce' ), $e->getCode(), $e->getMessage() );
			...
			if ( 'subscription' == pafw_get( $this->settings, 'management_batch_key', 'subscription' ) && $subscription && $subscription->get_payment_method() == $this->id ) {
				try {
					...
					if ( ! empty( $bill_key ) ) {
						...
						$this->clear_bill_key( $subscription, pafw_get_object_property( $subscription, 'customer_id' ) );
		...
		public function clear_bill_key( $subscription, $user_id ) {
			if ( $subscription ) {
				if ( is_callable( array ( $subscription, 'delete_meta_data' ) ) ) {
					...
					delete_post_meta( pafw_get_object_property( $subscription, 'id' ), $this->get_subscription_meta_key( 'auth_date' ) );
					delete_post_meta( pafw_get_object_property( $subscription, 'id' ), $this->get_subscription_meta_key( 'bill_key' ) );
					delete_post_meta( pafw_get_object_property( $subscription, 'id' ), $this->get_subscription_meta_key( 'card_code' ) );
					delete_post_meta( pafw_get_object_property( $subscription, 'id' ), $this->get_subscription_meta_key( 'card_name' ) );
					delete_post_meta( pafw_get_object_property( $subscription, 'id' ), $this->get_subscription_meta_key( 'auth_date' ) );
					delete_post_meta( pafw_get_object_property( $subscription, 'id' ), $this->get_subscription_meta_key( 'card_num' ) );
				}
			}
			if ( $user_id > 0 && 'user' == pafw_get( $this->settings, 'management_batch_key', 'subscription' ) ) {
				delete_user_meta( $user_id, $this->get_subscription_meta_key( 'auth_date' ) );
		...
		public function register_card() {
			...
			try {
				if ( isset( $_REQUEST['data'] ) ) {
					...
					if ( $this->issue_bill_key( null, $payment_info ) ) {
		...
		public function delete_card() {
			...
			try {
				...
				$bill_key = get_user_meta( $user_id, $this->get_subscription_meta_key( 'bill_key' ) );

				if ( ! empty( $bill_key ) ) {
					$this->cancel_bill_key( $bill_key );
					$this->clear_bill_key( null, $user_id );
				} else {
		...
		protected function get_vat_rate() {
			return array (
				'rate'     => 10.0,
		...
		protected function calculate_tax( $amount ) {
			if ( $amount > 0 ) {
		...
		protected function get_total_tax( $order, $amount = 0 ) {
			if ( $order ) {
				if ( wc_tax_enabled() ) {
					$refund = PAFW()->get_refund_for_order( $order );
		...
		protected function get_tax_free_amount( $order, $amount = 0 ) {
			$tax_free_amount = 0;
		...
		protected function get_tax_amount( $order, $amount = 0 ) {
			$the_order = $order;
			$refund    = PAFW()->get_refund_for_order( $the_order );
			...
			return $order_total - $this->get_tax_free_amount( $order, $amount ) - $this->get_total_tax( $order, $amount );
		}
	}
}