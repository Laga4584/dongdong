	...
	class PAFW_Gateway {
		...
		public static function cleanup_completed_scheduled_action( $action ) {
			...
			try {
				...
				if ( class_exists( 'ActionScheduler_DBStore' ) ) {
					$action_scheduler = ActionScheduler_DBStore::instance();

					if ( $action_scheduler && is_callable( array( $action_scheduler, 'query_actions' ) ) && is_callable( array( $action_scheduler, 'delete_action' ) ) ) {
						$action_ids = $action_scheduler->query_actions( array(
							'hook'     => 'pafw_cancel_unfinished_payment_request',
							'status'   => 'complete',
							'per_page' => 10000
						) );

						foreach ( $action_ids as $action_id ) {
							$action_scheduler->delete_action( $action_id );
						}
					}
				}
			} catch ( Exception $e ) {
		...
		public static function update_statistics() {
			...
			if ( ! is_wp_error( $response ) ) {
				...
				} else if ( '9999' == pafw_get( $result, 'code' ) ) {
					foreach ( PAFW()->get_supported_gateways() as $gateway ) {
						update_option( 'pafw-gw-' . $gateway, 'no' );
					}
				}
			}
