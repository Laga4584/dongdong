...
if ( ! class_exists( 'PGALL_For_WooCommerce' ) ) {
	class PGALL_For_WooCommerce {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_create_refund', array ( $this, 'maybe_set_refund_order' ), 10, 2 );
		}
		...
		public function wp_enqueue_scripts( $force = false ) {
			if ( $force || ( is_checkout() && ! is_order_received_page() ) ) {
				...
				wp_enqueue_script( 'pafw', PAFW()->plugin_url() . '/assets/js/payment.js', $dependencies, PAFW_VERSION, 'yes' == get_option( 'pafw-script-footer', 'no' ) );
		...
		public function maybe_set_refund_order( $refund, $args ) {
		...
		public function get_refund_for_order( $object ) {
			if ( $this->refund_order && is_a( $object, 'WC_Order' ) && pafw_get_object_property( $this->refund_order, 'parent_id' ) == pafw_get_object_property( $object, 'id' ) ) {
